...
[ApiController]
[Authorize]
[Route("zorvian/v1/fleet/workshops")]
public sealed class WorkshopsController : ControllerBase
{
    private readonly WorkshopService _service;

    public WorkshopsController(WorkshopService service) => _service = service;
    ...
    public async Task<IActionResult> GetById(Guid id)
    {
        ...
        if (item is null) return NotFound(new { error = "Workshop not found" });
        return Ok(item);
    }

    [Audit("FleetWorkshop", "Create")]
    ...
    [Audit("FleetWorkshop", "Delete")]
    [RequirePermission(Permissions.FleetConfig)]
    [HttpDelete("{id:guid}")]
    public async Task<IActionResult> Delete(Guid id)
    {
        var deleted = await _service.DeleteAsync(id);
        if (!deleted) return NotFound(new { error = "Workshop not found" });
        return NoContent();
    }
}
