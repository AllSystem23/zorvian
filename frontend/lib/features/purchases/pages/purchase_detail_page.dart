...
final class _PurchaseDetailPageState extends ConsumerState<PurchaseDetailPage> {
  ...
  Widget build(BuildContext context) {
    ...
    if (_loading) return Scaffold(appBar: AppBar(title: const Text('Compra')), body: const Center(child: CircularProgressIndicator()));
    if (_error != null) return Scaffold(appBar: AppBar(title: const Text('Compra')), body: Center(child: Text(_error!, style: TextStyle(color: theme.colorScheme.error))));
    final d = _data!;
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(d.purchaseNumber),
        actions: d.status != 'cancelled'
            ? [IconButton(icon: const Icon(Icons.cancel_outlined), tooltip: 'Anular', onPressed: _cancel)]
            : null,
      ),
      body: ListView(
